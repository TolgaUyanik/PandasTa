# Completed work — PandasTa fork

Everything here is **finished**. It was moved out of `TODO.md` on 2026-09-12 so that file could go
back to being a list of what is left, which by then was one item.

**This is not a changelog and it is not decoration.** These blocks carry the measured evidence
behind every shipped and every DELETED column — the ρ values, the sample sizes, the reverts, and
the traps that cost a review round to find. Several of them are the only written record of why a
thing is *not* there, which is exactly the knowledge a future session destroys by re-doing the
work. `TODO.md` keeps a one-line index and points here.

Read `docs/IndicatorDictionary.md` for what currently ships, and the per-campaign measurement
docs (`TalibPortsMeasured.md`, `CandlePatternsMeasured.md`, `AltportPortsMeasured.md`,
`PineAlternatesMeasured.md`, `PineBuiltinsMeasured.md`, `MLCol1Screen.md`, `MLCol1Axes.md`) for
the per-column numbers.
...
**Verdict split:** deliberately NOT reproduced here. `TODO.md` is its single declared
home and `test_no_other_document_retypes_the_split` fails the moment a second document
carries it — which this archive did on its first draft, because the block was copied
verbatim. Four review rounds once went by with a stale split standing in three
documents; the guard is why that cannot recur.
...
- [x] **PINEBI-1b — Port the 16 `port` rows. ✅ DONE 2026-09-11. Zero `port` rows remain.**
  ROWS: none remaining — the CSV routes every one of the 16 to a terminal verdict.
  **9 shipped · 7 built, measured and reverted on Gate E · 0 outstanding.**

  **SHIPPED (9), wired through all five touch points, MPL-2.0 © TradingView in every docstring
  except `kcw`'s** (a Pine CORE built-in, which carries none):
  `szo`, `vstop`, `vstop2`, `wpo` ship CLEAN (max ρ 0.7289 / 0.7503 / 0.7492 / 0.6477);
  `frama`, `ht`, `pzo`, `stc_tv`, `vzo` ship WITH DISCLOSURE (0.8540 / 0.7660 / 0.7877 /
  0.8136 / 0.7760). Per-column receipts: `docs/PineBuiltinsMeasured.md`, regenerated by
  `../Backtesting/scripts/analysis/measure_pinebi1b_gate_e.py --refresh`.

  🔴 **REVERTED (7) — built, measured, deleted.** `CLAUDE.md`: *"Deleting an over-correlated
  column you just built is the expected outcome, not a failure."* Recorded in the CSV under its
  own verdict and in `REVERTED_ON_GATE_E` in the generator, **so the next session cannot re-port
  them**: `ift` (ρ +1.0000 vs `RSI`), `relativeVolume` (0.9980 vs `VOL_RATIO`), `williamsFractal`
  (0.9884 vs `FRACTAL_UP` — exactly the collision this block predicted), `rms` (0.9900),
  `kcw` (0.9843 vs `natr`), `rwi` (0.9399), `dm`/Demarker (0.8992).

  ⚠ **`ift`'s ρ = 1.0000 is STRUCTURAL, and it generalises past this task.** The inverse Fisher
  transform is `tanh` — strictly monotone — so it is **rank-identical to whatever drives it**, and
  Spearman is a rank correlation. No choice of driver changes that, and a tree that splits on
  thresholds gains literally nothing from a monotone re-expression of a column it already has.
  **Any strictly monotone transform of a shipped column is dead on arrival under this Gate E**, and
  measuring one is only worth it to record the fact.

  ⚠ **`dm` was reverted on the APPROXIMATE reading of the ship line.** `CLAUDE.md` writes
  *"ρ ≈ 0.9 → revert"*; the measurement is 0.8992. A hard 0.90 cutoff would have shipped it on the
  strength of the fourth decimal, which is the manufactured-band-gap trap this repo has already
  fallen into once.

  ⚠ **Two findings about the GATE, from the mutant tests, that outlive these ports:**
  1. **A sign-based indicator cannot be causality-tested with a multiplicative perturbation.**
     Scaling future closes by 1.3 leaves their ORDER unchanged, so `sign(change(close))` is
     unchanged and a genuinely non-causal `szo` mutant **escaped at 65 of 113 J values**. Mirroring
     helps except at bar `J`, where the mirror is the identity. Mutate the SMOOTHER instead.
  2. **"Mutant caught at every J" is unattainable for a CLAMPED or BINARY output**, and demanding
     it means either deleting the test or shipping a false green. `stc_tv` clamps to [0,100] and
     saturates (36/63); a binary column only shows a mutant where the flag flips. Both now carry a
     documented catch floor; the REAL indicator is still required to leak at ZERO J values, which
     is the assertion that actually protects causality.

  ⚠ **`stc` ships as `stc_tv`** — this fork's `stc` is a different calculation (no `d1`/`d2`, no
  clamp, and a `lowest_xmacd > 0` guard that freezes its first stochastic whenever the rolling MACD
  minimum is ≤ 0, the normal case for a zero-centred oscillator). Same `t3`/`t3_tv` precedent
  PINEBI-1c set. Recorded in `RENAMED_PORTS` so a name-matching classifier cannot call it a gap.

  ⚠ **Still blocked on data (2, unchanged):** the `port - blocked on data` rows stay with
  PINEBI-1d; they need a lower-timeframe frame the single-OHLCV contract cannot supply.
